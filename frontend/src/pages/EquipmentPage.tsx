...
export function EquipmentPage() {
  ...
  const renderHealthBadge = (e: Equipment) => {
    ...
    const status = e.health_status ?? e.alert_level ?? "unknown"

    switch (status) {

      case "healthy":
        return <span className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded">Healthy</span>

      case "warning":
        return <span className="px-2 py-1 text-xs bg-yellow-100 text-yellow-700 rounded">Warning</span>

      case "attention":
        return <span className="px-2 py-1 text-xs bg-orange-100 text-orange-700 rounded">Attention</span>

      case "critical":
        return <span className="px-2 py-1 text-xs bg-red-100 text-red-700 rounded">Critical</span>

      default:
        return <span className="px-2 py-1 text-xs bg-gray-100 text-gray-500 rounded">Unknown</span>
    }
  }

  const getHealthStripColor = (e: Equipment) => {

    const status = e.health_status ?? e.alert_level
  ...
  const renderStatusTags = (e: Equipment) => {
    ...
    return (
      <div className="flex flex-wrap gap-2 mt-2">
        ...
        {e.pm_alert === "pm_required" && (
          <span className="px-2 py-1 text-xs bg-red-100 text-red-700 rounded">
            ⚠ PM DUE
          </span>
        )}

        {e.remaining_operating_months !== null &&
          e.remaining_operating_months !== undefined &&
          e.remaining_operating_months <= 6 && (
  ...
  const getRemainingLife = (e: Equipment) => {
    ...
    if (e.lifecycle_type === "years") {
      if (e.remaining_life_years !== undefined && e.remaining_life_years !== null) {
        return `${e.remaining_life_years} years`
      }
      return "N/A"
    }

    if (e.lifecycle_type === "hours") {
      if (e.remaining_operating_months !== undefined && e.remaining_operating_months !== null) {
        return `${e.remaining_operating_months} months`
      }
      return "N/A"
    }
          ...
          {sortedItems.map(e => {
            ...
            return (
              ...
              >
                ...
                <div className={`w-2 ${getHealthStripColor(e)}`} />
                ...
                <div className="flex justify-between p-4 w-full">
                  ...
                  <div>
                    ...
                    <div className="font-bold text-lg">{e.equipment_name}</div>
